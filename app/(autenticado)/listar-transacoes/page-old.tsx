...
export default function PaginaListarTransacoes() {
  ...
  useEffect(() => {
    const loadTransactions = async () => {
      try {
        console.log('[listar-transacoes] Loading transactions...');
        const savedTransactions = localStorage.getItem('transactions');
        if (savedTransactions) {
          console.log('[listar-transacoes] Found saved transactions in localStorage');
          setTransactions(JSON.parse(savedTransactions));
        } else {
          console.log('[listar-transacoes] No saved transactions, fetching from /transactions.json');
          // Carregar do arquivo JSON public/transactions.json com timeout
          ...
          try {
            ...
            console.log('[listar-transacoes] Fetch status:', res.status);
            const data = await res.json();
            console.log('[listar-transacoes] Fetched data:', data);
            const transactions = data.transactions || [];
            setTransactions(transactions);
            localStorage.setItem('transactions', JSON.stringify(transactions));
            console.log('[listar-transacoes] Transactions loaded and saved to localStorage');
          } catch (fetchError) {
            clearTimeout(timeoutId);
            console.error('[listar-transacoes] Fetch error:', fetchError);
            setTransactions([]);
          }
        }
      } catch (error) {
        console.error('[listar-transacoes] Error loading transactions:', error);
        setTransactions([]);
      }
      setIsLoading(false);
    };
